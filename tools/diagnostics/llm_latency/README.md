...
```powershell
python .\tools\diagnostics\llm_latency\test_model_speed.py
```

### 5. 运行 vLLM 延迟测试

vLLM 密钥只从环境变量读取，不写入仓库：

```powershell
$env:VLLM_BASE_URL = "http://192.168.10.100:8000"
$env:VLLM_MODEL = "qwen3.6-35b-a3b"
$env:VLLM_API_KEY = "<你的密钥>"
python .\tools\diagnostics\llm_latency\test_vllm_latency.py
python .\tools\diagnostics\llm_latency\test_streaming_latency.py
```
