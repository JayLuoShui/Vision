...
QListView, QTableWidget {
    background-color: #181a1f;
    alternate-background-color: #222833;
    border: 1px solid #3a4658;
...
class ImageCanvas(QWidget):
    ...
    def set_mode(self, mode: str) -> None:
        if mode in ("box", "polygon") and mode != self.mode:
            self.mode = mode
            self._reset_interaction()
            self.update()
...
class MainWindow(QMainWindow):
    ...
    def build_left_panel(self) -> QWidget:
        ...
        self.mode_combo.addItems(["检测框 (detect)", "分割多边形 (segment)"])
    ...
    def build_right_panel(self) -> QWidget:
        ...
        canvas_hint = QLabel(
            "左键：画框/选中  Esc：撤回当前点/取消当前绘制  右键/回车/双击：完成多边形  中键/Ctrl+左键：拖拽"
        )
        ...
        self.box_table.setMaximumHeight(180)

        layout.addLayout(nav)
        layout.addWidget(self.canvas, 1)
        layout.addWidget(canvas_hint)
        layout.addWidget(QLabel("当前图片标注"))
    ...
    def current_task(self) -> str:
        return "segment" if self.mode_combo.currentIndex() == 1 else "detect"

    def on_mode_changed(self, idx: int) -> None:
        mode = "polygon" if idx == 1 else "box"
    ...
    def goto_image(self, row: int) -> None:
        ...
        self.info_label.setText(f"{row + 1}/{len(self.image_paths)}  {path.name}  {w}×{h}  框数:{len(anns)}")
    ...
    def copy_previous_boxes(self) -> None:
        ...
        self.append_log(f"已复制上一张标注：{len(copied)} 个")
