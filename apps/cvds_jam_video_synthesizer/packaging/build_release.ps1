...
& $PyInstaller --noconfirm (Join-Path $ScriptDir "CVDS_Jam_Video_Synthesizer.spec") --workpath $BuildDir --distpath (Join-Path $Root "dist")
...
if (Test-Path (Join-Path $AppDir "docs")) {
    Copy-Item -LiteralPath (Join-Path $AppDir "docs") -Destination (Join-Path $DistDir "docs") -Recurse -Force
}
