...
from app.main import _resolve_config
...
def test_to_yaml_writes_unicode_as_utf8_text(tmp_path):
    ...
    assert b"_config_dir" not in raw


def test_relative_model_path_resolves_from_config_file_directory(tmp_path):
    ...
    assert config.model.model_path == "../weights/model"
    assert config.get_model_path() == str(model_dir.resolve())


def test_default_config_is_loaded_independently_of_working_directory(
    tmp_path,
    monkeypatch,
):
    monkeypatch.chdir(tmp_path)

    config = _resolve_config("config/config.yaml")

    assert [region.name for region in config.ignore_regions] == ["osd_text"]
