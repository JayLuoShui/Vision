...
$VisionRoot = (Resolve-Path (Join-Path $ProjectRoot "..\..")).Path
$Python = if ($Python) {
    $Python
} elseif ($env:DWS_SERVICE_PYTHON) {
    $env:DWS_SERVICE_PYTHON
} else {
    Join-Path $VisionRoot ".venv\Scripts\python.exe"
}
$ModelPath = if ($ModelPath) {
    $ModelPath
} elseif ($env:DWS_SERVICE_MODEL_PATH) {
    $env:DWS_SERVICE_MODEL_PATH
} else {
    Join-Path $VisionRoot "weights\yolo26s-seg-wds-1024-best_int8_openvino_model"
}
...
Invoke-Checked "Ruff" {
    & $Python -m ruff check (Join-Path $ProjectRoot "app") (Join-Path $ProjectRoot "scripts") (Join-Path $ProjectRoot "tests")
}
...
Copy-Item -LiteralPath (Join-Path $ProjectRoot "docs\WINDOWS_USER_GUIDE.md") -Destination (Join-Path $ReleaseRoot "UserGuide.md") -Force
