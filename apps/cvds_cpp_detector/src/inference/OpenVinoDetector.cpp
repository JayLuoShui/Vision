...
YoloOutputLayout readOutputLayoutFromMetadata(const QFileInfo& xmlInfo) {
    QFile metadata(xmlInfo.dir().filePath("metadata.yaml"));
    if (!metadata.open(QIODevice::ReadOnly | QIODevice::Text)) return YoloOutputLayout::Auto;
...
bool OpenVinoDetector::load(const QString& modelPath, const QString& device, int inputSize, QString* error) {
    ...
    try {
        QString xmlPath = modelPath.trimmed();
        if (xmlPath.isEmpty()) {
            if (error) *error = "OpenVINO 模型路径不能为空";
            return false;
        }

        QFileInfo info(xmlPath);
        if (info.isDir()) {
            const QStringList xmlFiles = QDir(xmlPath).entryList(
            ...
            if (xmlFiles.size() != 1) {
                if (error) *error = "OpenVINO 模型目录必须且只能包含一个 .xml 文件";
                return false;
            }
            xmlPath = QDir(xmlPath).filePath(xmlFiles.first());
        }
        const QFileInfo xmlInfo(xmlPath);
        if (!xmlInfo.isFile() || !xmlPath.endsWith(".xml", Qt::CaseInsensitive)) {
            if (error) *error = "运行端只支持 OpenVINO IR .xml 或模型目录";
            return false;
        }
        const QString binPath = xmlInfo.dir().filePath(xmlInfo.completeBaseName() + ".bin");
        if (!QFileInfo(binPath).isFile()) {
            if (error) *error = "OpenVINO IR 缺少同名 .bin 文件";
            return false;
        }
        outputLayout_ = readOutputLayoutFromMetadata(xmlInfo);

        auto model = core_.read_model(xmlPath.toStdString());
        if (model->inputs().size() != 1 || model->outputs().empty()) {
            if (error) *error = "仅支持单输入、至少单输出的 YOLO detect OpenVINO 模型";
            return false;
        }

        const ov::PartialShape inputShape = model->input().get_partial_shape();
        if (inputShape.rank().is_static() && inputShape.rank().get_length() != 4) {
            if (error) *error = "模型输入必须是四维 NCHW 张量";
            return false;
        }
        ...
        input_ = model->input();
        model->reshape({{input_.get_any_name(), ov::PartialShape(
...
DetectionResults OpenVinoDetector::infer(const cv::Mat& frame, float confidence, float iou, int classFilterId, QString* error) {
    ...
    try {
        ...
        inferRequest_.set_tensor(input_, inputTensor);
        inferRequest_.infer();
        ...
        const size_t outputCount = compiledModel_.outputs().size();
        for (size_t i = 0; i < outputCount; ++i) {
            const ov::Tensor outputTensor = inferRequest_.get_output_tensor(i);
            DetectionResults parsed = parseOutput(outputTensor, meta, config);
...
DetectionResults OpenVinoDetector::parseOutput(const ov::Tensor& output, const LetterBoxInfo& meta, const YoloPostprocessConfig& config) const {
    ...
    if (output.get_element_type() == ov::element::f16) {
        ...
        for (size_t i = 0; i < total; ++i) buffer[i] = static_cast<float>(src[i]);
    } else if (output.get_element_type() == ov::element::i64) {
        const auto* src = output.data<const int64_t>();
        for (size_t i = 0; i < total; ++i) buffer[i] = static_cast<float>(src[i]);
