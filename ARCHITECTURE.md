...
- `DWSVisionCountService/app/windows_app.py`：Windows 视觉计数控制台，负责服务状态、运行参数保存和完整重启。
- `DWSVisionCountService/app/roi_canvas.py`：选择现场图片并绘制检测矩形、输送带多边形和忽略矩形。
...
- YOLOMask 分割训练使用 RTX 4050 6GB 对应的 `yolo26s-seg.pt + imgsz=960 + batch=2`，在不爆显存的前提下优先保证小包裹分割性能。
